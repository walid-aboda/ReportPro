...
{
    public partial class frmAddSalesDelevry : Form
    {
        ...
        {
            DataTable dt_ = dal.getDataTabl_1("select * from "+dal.db1+ ".dbo.sales_delevry where D_CODE like '%" + txtCode.Text + "%' and D_Name like '%" + txtDesc.Text + "%' and D_NAME_E like '%" + txtDesc_L.Text + "%'");
            dt.Clear();
            int i = 0;
            DataRow row = null;
        ...
        private void fillDG()
        {
            DataTable dt_ = dal.getDataTabl_1("select * from  " + dal.db1 + ".dbo.sales_delevry");
            dt.Clear();
            int i = 0;
            DataRow row = null;
            foreach (DataRow r in dt_.Rows)
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (txtCode.Text != string.Empty && txtDesc.Text != string.Empty)
                ...
                {
                    dal.Execute_1("update  " + dal.db1 + ".dbo.sales_delevry set D_Name ='" + txtDesc.Text + "',D_NAME_E ='" + txtDesc_L.Text + "' where D_CODE = '" + txtCode.Text + "'");
